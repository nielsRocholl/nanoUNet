...
maps OUTPUT positions to INPUT positions via
    input_voxel(o) = (o - (patch_size-1)/2) @ affine + center_location_in_pixels - 0.5
(the -0.5 comes from new_center using shape/2 while the final pixel conversion uses (shape-1)/2 --
verified numerically against SpatialTransform._apply_to_image on single-voxel test volumes, see
the equivalence test in the task this file was written for). Tracking a point forward through the
same sampled transform needs the INVERSE:
    o = (p - center_location_in_pixels + 0.5) @ inv(affine) + (patch_size-1)/2
When affine is None, _apply_to_image takes the discrete crop_tensor() fast path instead of
grid_sample (no interpolation, no -0.5 term); we replicate its integer indexing (floor +
floor-division) exactly so the CT channels and the points agree to sub-voxel precision.
"""
...
def _invert_spatial(points: np.ndarray, patch_size, params: dict) -> np.ndarray:
    ...
    out = (points - center + 0.5) @ inv_affine + (ps - 1.0) / 2.0
