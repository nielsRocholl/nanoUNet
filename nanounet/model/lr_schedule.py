"""Polynomial LR + stretched-tail variant (nnU-Net), with optional linear warmup.

warmup_epochs=0 (the default) makes both schedulers bit-for-bit identical to the pre-warmup
code -- the warmup branch is simply unreachable. With warmup_epochs=N, the first N steps ramp
linearly to initial_lr and the existing formula then runs unchanged against the same step index
(warmup replaces the first N values, it does not delay the rest)."""
...
class PolyLRScheduler(_LRScheduler):
    ...
    def step(self, current_step=None):
        ...
        if self.warmup_epochs > 0 and current_step < self.warmup_epochs:
            new_lr = self.initial_lr * (current_step + 1) / self.warmup_epochs
        else:
