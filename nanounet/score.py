...
def _clicks(path: str) -> dict[int, tuple[int, int, int]]:
    ...
    for item in pts:
        ...
        p = item["point"]
        if lid not in out:
            out[lid] = (int(round(p[2])), int(round(p[1])), int(round(p[0])))
    return out
...
def score_case(case_id: str, pred_path: str, gt_path: str, clicks_json: str) -> dict:
    ...
    for lid, (z, y, x) in clicks.items():
        ...
        if not gt_i.any():
            n_skip += 1
            continue
        pred_i, j = pred_cc_at(pred, z, y, x), None
        j = iou(gt_i, pred_i)
...
def _dsc_cell(v) -> str:
    if isinstance(v, float) and v != v:
        return "—"
    s = f"{v:.3f}"
    return f"[red]{s}[/red]" if v < 0.10 else (f"[green]{s}[/green]" if v >= 0.70 else s)
...
def report(rows: list[dict]) -> None:
    ...
    cprint(t)
    vol = float(np.mean([r["volume_dice"] for r in rows]))
    dsc = float(np.nanmean([r["dsc"] for r in rows]))
    nsd_m = float(np.nanmean([r["nsd"] for r in rows]))
    ldr = float(np.nanmean([r["ldr"] for r in rows]))
    n_les, n_skip = sum(r["n"] for r in rows), sum(r["n_skip"] for r in rows)
    all_d = [L["dsc"] for r in rows for L in r["lesions"]]
    macro = float(np.nanmean(all_d)) if all_d else float("nan")
    cprint(Panel(
        f"Dice vol     {_f(vol)}    case-mean whole-volume (not comparable to LongiSeg)\n"
        f"DSC          {_f(dsc)}    case-mean per-lesion  ← vs LongiSeg verified ~0.737\n"
        f"NSD          {_f(nsd_m)}    1 mm surface\n"
        f"LDR          {_f(ldr)}    IoU > {IOU_HIT}\n"
        f"lesions      {n_les} scored, {n_skip} empty-GT skipped",
        title="LongiSeg lesion metrics", border_style="cyan"))
    cprint(f"[dim]lesion-macro DSC {_f(macro)} (every lesion equal; headline is case-mean)[/dim]")
...
def write(rows: list[dict], path: str) -> None:
    ...
    all_d = [L["dsc"] for r in rows for L in r["lesions"]]
    payload = {
        "protocol": "longiseg_lesion_v1", "nsd_tol_mm": NSD_TOL_MM, "ldr_iou": IOU_HIT,
        "overall": {
            "n_cases": len(rows), "n_lesions": sum(r["n"] for r in rows),
            "n_skipped_empty_gt": sum(r["n_skip"] for r in rows),
            "volume_dice_mean": _jsonable(float(np.mean([r["volume_dice"] for r in rows]))),
            "dsc_case_mean": _jsonable(float(np.nanmean([r["dsc"] for r in rows]))),
            "nsd_case_mean": _jsonable(float(np.nanmean([r["nsd"] for r in rows]))),
            "ldr_case_mean": _jsonable(float(np.nanmean([r["ldr"] for r in rows]))),
            "dsc_lesion_macro": _jsonable(float(np.nanmean(all_d)) if all_d else float("nan")),
        },
